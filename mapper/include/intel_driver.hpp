...
namespace intel_driver
{
	...
	{
		uint64_t case_number; // payson1337
		uint64_t reserved1;
		uint64_t reserved2;
	...
	bool CallKernelFunction(HANDLE device_handle, T* out_result, uint64_t kernel_function_address, const A ...arguments) {
		...
		if (ntdll == 0) {
			Log(L"[Saturn] Failed to load ntdll.dll" << std::endl);
			return false;
		}
		...
		if (!NtAddAtom)
		{
			Log(L"[Saturn] Failed to get export ntdll.NtAddAtom" << std::endl);
			return false;
		}
		...
		*(uint64_t*)&kernel_injected_jmp[2] = kernel_function_address;
		...
		if (original_kernel_function[0] == kernel_injected_jmp[0] &&
			...
			Log(L"[Saturn] FAILED!: The code was already hooked!! another instance of saturn running?!" << std::endl);
			return false;
		}
