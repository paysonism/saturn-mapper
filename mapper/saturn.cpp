...
uint64_t saturn::AllocMdlMemory(HANDLE iqvw64e_device_handle, uint64_t size, uint64_t* mdlPtr) {
	...
	if (!mdl) {
		Log(L"[Saturn] Can't allocate pages for mdl" << std::endl);
		return { 0 };
	}

	uint32_t byteCount = 0;
	if (!intel_driver::ReadMemory(iqvw64e_device_handle, mdl + 0x028 /*_MDL : byteCount*/, &byteCount, sizeof(uint32_t))) {
		Log(L"[Saturn] Can't read the _MDL : byteCount" << std::endl);
		return { 0 };
	}

	if (byteCount < size) {
		Log(L"[Saturn] Couldn't allocate enough memory, cleaning up" << std::endl);
	...
	if (!mappingStartAddress) {
		Log(L"[Saturn] Can't set mdl pages cache, cleaning up." << std::endl);
		intel_driver::MmFreePagesFromMdl(iqvw64e_device_handle, mdl);
		intel_driver::FreePool(iqvw64e_device_handle, mdl);
		return { 0 };
	}
	...
	if (!result) {
		Log(L"[Saturn] Can't change protection for mdl pages, cleaning up" << std::endl);
		intel_driver::MmUnmapLockedPages(iqvw64e_device_handle, mappingStartAddress, mdl);
		intel_driver::MmFreePagesFromMdl(iqvw64e_device_handle, mdl);
		intel_driver::FreePool(iqvw64e_device_handle, mdl);
		return { 0 };
	}
	Log(L"[Saturn] Allocated pages for mdl" << std::endl);
...
uint64_t saturn::AllocIndependentPages(HANDLE device_handle, uint32_t size)
{
	...
	if (!base)
	{
		Log(L"[Saturn] Error allocating independent pages" << std::endl);
		return 0;
	}

	if (!intel_driver::MmSetPageProtection(device_handle, base, size, PAGE_EXECUTE_READWRITE))
	{
		Log(L"[Saturn] Failed to change page protections" << std::endl);
...
uint64_t saturn::MapDriver(HANDLE iqvw64e_device_handle, BYTE* data, ULONG64 param1, ULONG64 param2, bool free, bool destroyHeader, AllocationMode mode, bool PassAllocationAddressAsFirstParam, mapCallback callback, NTSTATUS* exitCode) {
	...
	if (!nt_headers) {
		Log(L"[Saturn] Invalid format of PE image" << std::endl);
		return 0;
	}

	if (nt_headers->OptionalHeader.Magic != IMAGE_NT_OPTIONAL_HDR64_MAGIC) {
		Log(L"[Saturn] Image is not 64 bit" << std::endl);
		return 0;
	}
	...
	if (!kernel_image_base) {
		Log(L"[Saturn] Failed to allocate remote image in kernel" << std::endl);
	...
	do {
		Log(L"[Saturn] Image base has been allocated at 0x" << reinterpret_cast<void*>(kernel_image_base) << std::endl);
		...
		if (destroyHeader) {
			...
			Log(L"[Saturn] Skipped 0x" << std::hex << TotalVirtualHeaderSize << L" bytes of PE Header" << std::endl);
		}
		...
		{
			Log(L"[Saturn] Failed to fix cookie" << std::endl);
			return 0;
		}

		if (!ResolveImports(iqvw64e_device_handle, portable_executable::GetImports(local_image_base))) {
			Log(L"[Saturn] Failed to resolve imports" << std::endl);
		...
		if (!intel_driver::WriteMemory(iqvw64e_device_handle, realBase, (PVOID)((uintptr_t)local_image_base + (destroyHeader ? TotalVirtualHeaderSize : 0)), image_size)) {
			Log(L"[Saturn] Failed to write local image to remote image" << std::endl);
			kernel_image_base = realBase;
			break;
		}
		...
		Log(L"[Saturn] Calling DriverEntry 0x" << reinterpret_cast<void*>(address_of_entry_point) << std::endl);

		if (callback) {
			if (!callback(&param1, &param2, realBase, image_size, mdlptr)) {
				Log(L"[Saturn] Callback returns false, failed!" << std::endl);
				kernel_image_base = realBase;
				break;
			}
		}
		...
		if (!intel_driver::CallKernelFunction(iqvw64e_device_handle, &status, address_of_entry_point, (PassAllocationAddressAsFirstParam ? realBase : param1), param2)) {
			Log(L"[Saturn] Failed to call driver entry" << std::endl);
			kernel_image_base = realBase;
			break;
		}
		...
		Log(L"[Saturn] DriverEntry returned 0x" << std::hex << status << std::endl);

		if (free) {
			Log(L"[Saturn] Freeing memory" << std::endl);
			...
			if (free_status) {
				Log(L"[Saturn] Memory has been released" << std::endl);
			}
			else {
				Log(L"[Saturn] WARNING: Failed to free memory!" << std::endl);
			}
		}
...
bool saturn::FixSecurityCookie(void* local_image, uint64_t kernel_image_base)
{
	...
	{
		Log(L"[Saturn] Load config directory wasn't found, probably StackCookie not defined, fix cookie skipped" << std::endl);
		return true;
	}
	...
	if (!stack_cookie)
	{
		Log(L"[Saturn] StackCookie not defined, fix cookie skipped" << std::endl);
		return true;
	}

	stack_cookie = stack_cookie - (uintptr_t)kernel_image_base + (uintptr_t)local_image; 
	if (*(uintptr_t*)(stack_cookie) != 0x2B992DDFA232) {
		Log(L"[Saturn] StackCookie already fixed!? this probably wrong" << std::endl);
		return false;
	}

	Log(L"[Saturn] Fixing stack cookie" << std::endl);
...
bool saturn::ResolveImports(HANDLE iqvw64e_device_handle, portable_executable::vec_imports imports) {
	for (const auto& current_import : imports) {
		...
		if (!Module) {
			...
			std::cout << "[Saturn] Dependency " << current_import.module_name << " wasn't found" << std::endl;
#endif
			return false;
		}
		...
		for (auto& current_function_data : current_import.function_datas) {
			...
			if (!function_address) {
				if (Module != intel_driver::ntoskrnlAddr) {
					...
					if (!function_address) {
#if !defined(DISABLE_OUTPUT)
						std::cout << "[Saturn] Failed to resolve import " << current_function_data.name << " (" << current_import.module_name << ")" << std::endl;
#endif
						return false;
					}
				}
