...
bool service::RegisterAndStart(const std::wstring& driver_path) {
	...
	if (status != ERROR_SUCCESS) {
		Log("[Saturn] Can't create service key" << std::endl);
		return false;
	}
	...
	if (status != ERROR_SUCCESS) {
		...
		Log("[Saturn] Can't create 'ImagePath' registry value" << std::endl);
		return false;
	}
	...
	if (status != ERROR_SUCCESS) {
		RegCloseKey(dservice);
		Log("[Saturn] Can't create 'Type' registry value" << std::endl);
		return false;
	}
	...
	Log("[Saturn] NtLoadDriver Status 0x" << std::hex << Status << std::endl);
...
bool service::StopAndRemove(const std::wstring& driver_name) {
	...
	Log("[Saturn] NtUnloadDriver Status 0x" << std::hex << st << std::endl);
	if (st != 0x0) {
		Log("[Saturn] Driver Unload Failed!!" << std::endl);
